...
'06':{title:'一份教材生出 5 種遊戲：生成方法',tag:'GAME GENERATION / FIVE FORMS',accent:'#e3a62f',lede:'先決定要練什麼，再從五種玩法選一款，複製提示詞做出可以試玩的族語教材。',goals:['理解如何把練習做成有目標、回饋與重試的小任務','依認識詞彙、練習拼寫、練習造句、理解情境與進階猜詞選玩法','使用老師確認的族語與方言語料，完成一款遊戲並試玩'],steps:['說清楚希望學生練習哪一件事','從翻牌配對、防衛戰、語序火車、山林冒險與 Wordle 選一款','準備老師確認的族語、方言別、翻譯與答案','複製該款完整提示詞，貼上語料，先做出可以操作的版本','試玩、記錄問題，一次修改一項再測試'],practice:'選一款玩法，使用自己準備的語料製作遊戲，請同學試玩並記錄需要修正的地方。',output:'一款可試玩的族語遊戲與修正紀錄',prompts:['請依學習目標整理五種玩法的簡短提案：認識詞彙用翻牌配對、練習拼寫用防衛戰、練習造句用語序火車、理解情境用山林冒險、進階猜詞用 Wordle。每款只列學生要做什麼、會得到什麼回饋，以及需要準備的語料。五款是可選玩法，不是連續關卡。','請只使用我提供且已確認的族語、方言別、翻譯、答案與文化說明。資料不足時列出缺少欄位，不要自行翻譯或編造；再依我的學習目標推薦先做的一款玩法。','請把我選的玩法縮小成可以試玩的版本，保留開始、作答、回饋與重試。列出我應該如何檢查答案、按鈕和手機操作，讓我記錄問題後逐項修正。'],source:'族語教學遊戲提示詞與課堂實作'},
...
'08':{title:'LINE 教學小幫手：行事曆與單詞查詢',tag:'LINE + GAS / GOOGLE WORKSPACE',accent:'#147d7e',lede:'從 LINE 官方帳號申請開始，完成 Google 行事曆新增或太魯閣語單詞查詢。',goals:['建立 LINE 官方帳號並啟用 Messaging API','理解 Token、Webhook URL 與綁定碼','將 Apps Script 部署成 Web App 並完成端對端驗收'],steps:['選擇行事曆或單詞小幫手','建立練習用 LINE 官方帳號','取得 Channel access token 並存入 Script Properties','準備 Apps Script 程式與 Google 資料','部署 /exec 並設定 LINE Webhook','使用真實 LINE 訊息完成測試'],practice:'完成其中一套 LINE 小幫手，附上 LINE 回覆、Google 端結果與已遮蔽憑證的測試證據。',output:'可用的 LINE 行事曆或單詞小幫手、測試紀錄與安全檢查',prompts:['請依照我提供的無痕 /exec 結果、Webhook 設定與已移除憑證的執行記錄，找出 LINE 沒有回覆的最可能原因；先列檢查順序，再提供最小修正。','請先檢查我的單詞列表工作表名稱、A 到 H 欄位、初始化筆數與一筆匿名化資料，再判斷為什麼查不到詞；不要先重寫程式。','請只新增我指定的一項功能，保留目前可用流程，先列出會修改的函式、風險與至少六個測試案例。'],source:'LINE Messaging API 與 Google Apps Script 官方文件；太魯閣語詞料需由族語教師或資料權責者確認。'},
