...
const unlimitedSession = core.normalizeSession({
  ...
  classroom: { status: "open", opens_at: "2026-09-04T11:00:00Z", closes_at: "2026-09-04T14:00:00Z" },
  remaining: { text: null, image: null },
  classroom_remaining: { text: null, image: null }
});
assert.deepEqual(unlimitedSession.remaining, { text: null, image: null }, "unlimited quotas must not be normalized to zero");
...
assert.match(html, /<dt>圖片生成<\/dt><dd id="personal-image-quota">不限次<\/dd>/, "image generation must be labelled as unlimited");
assert.doesNotMatch(html, /剩餘圖片生成次數/, "the removed image counter label must not return");
assert.match(html, /AI 產出先當草稿/, "AI output must be presented as a draft");
assert.doesNotMatch(html, /扣掉\s*1\s*次圖片生成次數/, "image generation must not claim to consume a total quota");
...
assert.doesNotMatch(availabilityBlock, /imageRemaining|remaining\.image/, "image generation availability must not depend on a total image quota");
...
assert.match(availabilityBlock, /generate-image-button"\)\.disabled\s*=\s*textBusy\s*\|\|\s*imageBusy/, "image generation must only be disabled while generation is busy");
assert.match(app, /personal-text-quota"\)\.textContent\s*=\s*"不限次"/, "quota display must always show unlimited AI suggestions");
assert.match(app, /personal-image-quota"\)\.textContent\s*=\s*"不限次"/, "quota display must always show unlimited image generation");
assert.doesNotMatch(app, /previewQuota\.image\s*-=|previewQuota\.image\s*<=\s*0/, "local preview must not decrement or block on an image total");
